...
export default function CourseDetails() {
  ...
  async function onEnroll() {
    try { const r = await api.enroll(course!._id); setMsg(r.message); }
  ...
  return (
    <main className="mx-auto max-w-4xl px-5 py-12">
      ...
      <p className="mt-1 text-sm">by {course.instructor?.name} · {course.lessons.length} lessons · {Math.round(mins / 60)}h · {course.students.length} enrolled</p>
      <p className="mt-6 max-w-2xl">{course.description}</p>

      <div className="mt-8 flex items-center gap-5">
        <span className="text-2xl font-bold text-cream">₹{course.price}</span>
        {user
          ? <button onClick={onEnroll} className="rounded-xl bg-amber px-6 py-2.5 font-semibold text-ink hover:bg-amber-dark">Enroll</button>
